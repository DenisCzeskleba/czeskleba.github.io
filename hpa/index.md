      ...
      <div class="hpa-stage-header">
        ...
        <div class="hpa-results-grid" aria-label="Diffusion model results">
          <article class="hpa-result-card">
            <h3>Breakthrough <span class="hpa-result-hint" tabindex="0" title="Shown for reference. Diagnostic confidence and Global Transient Fit seeding currently exclude breakthrough because the ISO linear-rise extrapolation is still quite brittle. This method is often reported as less reliable and remains a work in progress here.">?</span></h3>
          ...
          <article class="hpa-result-card hpa-result-card-fit">
            ...
            <button type="button" class="hpa-button is-secondary hpa-fit-apply" id="hpa-fit-apply" title="Apply the current Global Transient Fit t0 correction to the Start Time Offset slider.">Apply t0</button>
  ...
  <aside class="hpa-help-panel" role="dialog" aria-modal="true" aria-label="How HPA works">
    ...
    <div class="hpa-help-body">
      ...
      <div>
        ...
        <ul class="hpa-help-list">
          <li><strong>Breakthrough</strong> follows the ISO 17081 definition: it extrapolates the approximately linear rising part of the transient back to zero permeation signal. In this implementation the fit is evaluated on the baseline-corrected, normalized curve, which is equivalent to extrapolating the raw permeation current above baseline. The reported value is shown for reference, but the diagnostic confidence and the global-fit seed do not use it because the linear-rise extrapolation is still quite sensitive to rise-window selection.</li>
          ...
          <li><strong>Global Transient Fit</strong> searches for one constant <code>D</code> and one relative <code>t<sub>0</sub></code> correction on top of the current <strong>Start Time Offset</strong>. The card reports that correction as a relative value, and <strong>Apply t0</strong> adds it to the current slider setting.</li>
        </ul>
        ...
        <p>The preview table shows <code>D<sub>app</sub></code> in mm&sup2;/s for readability and includes an <strong>Origin</strong> column that distinguishes measured rows from reconstructed prepended baseline. The CSV export writes <code>D<sub>app</sub></code> in m&sup2;/s, so the exported numbers look different even though they represent the same quantity.</p>
      </div>
