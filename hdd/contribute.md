...
<form class="hdd-contrib-form" id="hydrogen-contribution-form" action="https://formspree.io/f/xjgawgej" method="POST" novalidate>
  ...
  <div class="hdd-contrib-local">
    <label><input type="checkbox" id="hdd-local-mode" title="Generate JSON locally without sending a submission. Uncheck to send the form." checked /> Local test mode (JSON only)</label>
    <label><input type="checkbox" id="hdd-skip-validation" title="Bypass every client-side validation check and send the submission anyway." /> Skip validation checks (send anyway)</label>
  ...
  <p class="hdd-contrib-note">Uncheck local test mode before submitting, otherwise the form will only generate JSON in the browser.</p>
